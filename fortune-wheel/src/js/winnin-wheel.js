...
export default class Winwheel {
		...
		if (this.ctx) {
			...
			if (this.segments) {
				...
				for (let x = 1; x <= this.numSegments; x++) {
					...
					this.seg = this.segments[x];

					// Check image has loaded so a property such as height has a value.
					if (this.seg.imgData.height) {
						...
						if (this.seg.imageDirection !== null) imageDirection = this.seg.imageDirection;
						else imageDirection = this.imageDirection;

						if (imageDirection == "S") {
							// Left set so image sits half/half over the 180 degrees point.
							imageLeft = this.centerX - this.seg.imgData.width / 2;
							...
							imageAngle = this.seg.startAngle + 180 + (this.seg.endAngle - this.seg.startAngle) / 2;
						} else if (imageDirection == "E") {
							// Left set so image starts and the center point.
							imageLeft = this.centerX;

							// Top is so that it sits half/half over the 90 degree point.
							imageTop = this.centerY - this.seg.imgData.height / 2;

							// Again get the angle in the center of the segment and add it to the rotation angle.
							// this time we need to add 270 to that to the segment is rendered the correct place.
							imageAngle = this.seg.startAngle + 270 + (this.seg.endAngle - this.seg.startAngle) / 2;
						} else if (imageDirection == "W") {
							// Left is the centerX minus the width of the image.
							imageLeft = this.centerX - this.seg.imgData.width;
							...
							imageAngle = this.seg.startAngle + 90 + (this.seg.endAngle - this.seg.startAngle) / 2;
							...
							imageTop = this.centerY - this.seg.imgData.height;

							// Angle to draw the image is its starting angle + half its size.
							// this sits it half/half over the center angle of the segment.
							imageAngle = this.seg.startAngle + (this.seg.endAngle - this.seg.startAngle) / 2;
						}
						...
						// Draw the image.
						this.ctx.drawImage(this.seg.imgData, imageLeft, imageTop);
		...
		if (this.ctx) {
			...
			if (this.segments) {
				...
				for (let x = 1; x <= this.numSegments; x++) {
					...
					if (this.seg.fillStyle !== null) fillStyle = this.seg.fillStyle;
					else fillStyle = this.fillStyle;

					this.ctx.fillStyle = fillStyle;

					if (this.seg.lineWidth !== null) lineWidth = this.seg.lineWidth;
					else lineWidth = this.lineWidth;

					this.ctx.lineWidth = lineWidth;

					if (this.seg.strokeStyle !== null) strokeStyle = this.seg.strokeStyle;
					...
					if (strokeStyle || fillStyle) {
						...
						this.ctx.arc(
							...
							this.centerY,
							this.outerRadius,
							this.degToRad(this.seg.startAngle + this.rotationAngle - 90),
							this.degToRad(this.seg.endAngle + this.rotationAngle - 90),
							false
						);
		...
		if (this.ctx) {
			...
			for (let x = 1; x <= this.numSegments; x++) {
				...
				if (this.seg.text) {
					...
					if (this.seg.textFontFamily !== null) fontFamily = this.seg.textFontFamily;
					else fontFamily = this.textFontFamily;
					if (this.seg.textFontSize !== null) fontSize = this.seg.textFontSize;
					else fontSize = this.textFontSize;
					if (this.seg.textFontWeight !== null) fontWeight = this.seg.textFontWeight;
					else fontWeight = this.textFontWeight;
					if (this.seg.textOrientation !== null) orientation = this.seg.textOrientation;
					else orientation = this.textOrientation;
					if (this.seg.textAlignment !== null) alignment = this.seg.textAlignment;
					else alignment = this.textAlignment;
					if (this.seg.textDirection !== null) direction = this.seg.textDirection;
					else direction = this.textDirection;
					if (this.seg.textMargin !== null) margin = this.seg.textMargin;
					else margin = this.textMargin;
					if (this.seg.textFillStyle !== null) fillStyle = this.seg.textFillStyle;
					else fillStyle = this.textFillStyle;
					if (this.seg.textStrokeStyle !== null) strokeStyle = this.seg.textStrokeStyle;
					else strokeStyle = this.textStrokeStyle;
					if (this.seg.textLineWidth !== null) lineWidth = this.seg.textLineWidth;
					...
					for (let i = 0; i < lines.length; i++) {
						...
						if (direction == "reversed") {
							...
							if (orientation == "horizontal") {
								...
								var textAngle = this.degToRad(this.seg.endAngle - (this.seg.endAngle - this.seg.startAngle) / 2 + this.rotationAngle - 90 - 180);
								...
								if (alignment == "inner") {
									...
									if (fillStyle)
										this.ctx.fillText(
											lines[i],
											this.centerX - this.innerRadius - (this.outerRadius - this.innerRadius) / 2 - margin,
								...
								var textAngle = this.seg.endAngle - (this.seg.endAngle - this.seg.startAngle) / 2 - 180;
								...
								if (alignment == "outer") {
									...
									for (var c = lines[i].length - 1; c >= 0; c--) {
										this.character = lines[i].charAt(c);

										if (fillStyle) this.ctx.fillText(this.character, this.centerX + lineOffset, yPos);

										if (strokeStyle) this.ctx.strokeText(this.character, this.centerX + lineOffset, yPos);
									...
									// If there is more than one this.character in the text then an adjustment to the position needs to be done.
								...
								// as we do this by rotating the wheel small amounts adding each this.character.
								var anglePerChar = 0;
								var drawAngle = 0;

								// If more than one this.character in the text then...
								if (lines[i].length > 1) {
									// Text is drawn from the left.
									this.ctx.textAlign = "left";

									// Work out how much angle the text rendering loop below needs to rotate by for each this.character to render them next to each other.
									...
									drawAngle = this.seg.startAngle + ((this.seg.endAngle - this.seg.startAngle) / 2 - totalArc / 2);
								} else {
									// The initial draw angle is the center of the segment when only one this.character.
									drawAngle = this.seg.startAngle + (this.seg.endAngle - this.seg.startAngle) / 2;
								...
								for (c = lines[i].length; c >= 0; c--) {
									...
									// Rotate the wheel to the draw angle as we need to add the this.character at this location.
									this.ctx.translate(this.centerX, this.centerY);
									this.ctx.rotate(this.degToRad(drawAngle));
									this.ctx.translate(-this.centerX, -this.centerY);

									// Now draw the this.character directly below the center point of the wheel at the appropriate radius.
									// Note in the reversed mode we add the radius to the this.centerY instead of subtract.
									if (strokeStyle) this.ctx.strokeText(this.character, this.centerX, this.centerY + radius + lineOffset);

									if (fillStyle) this.ctx.fillText(this.character, this.centerX, this.centerY + radius + lineOffset);

									// Increment the drawAngle by the angle per this.character so next loop we rotate
									// to the next angle required to draw the this.character at.
							...
							if (orientation == "horizontal") {
								...
								var textAngle = this.degToRad(this.seg.endAngle - (this.seg.endAngle - this.seg.startAngle) / 2 + this.rotationAngle - 90);
								...
								// which causes the text to draw downwards or upwards one this.character after another.
								...
								var textAngle = this.seg.endAngle - (this.seg.endAngle - this.seg.startAngle) / 2;
								...
								if (alignment == "outer") {
									...
									// So logically we have to put the middle this.character of the text in the center then put the others each side of it.
									...
									// centerAdjustment included to correctly position texts with more than one this.character over the center.
								...
								// ----------------------
								// Now the drawing itself.
								// Loop for each this.character in the text.
								for (c = 0; c < lines[i].length; c++) {
									...
									// Now draw the this.character directly above the center point of the wheel at the appropriate radius.
									if (strokeStyle) this.ctx.strokeText(this.character, this.centerX, this.centerY - radius + lineOffset);
	...
	startAnimation() {
		if (this.animation) {
			...
			properties["onComplete"] = () => winwheelStopAnimation(true); // Call function to perform actions when animation has finished.
	...
	computeAnimation() {
		if (this.animation) {
			...
			if (this.animation.type == "spinOngoing") {
				// ?
				this.animation.propertyValue = this.animation.stopAngle;
...
class Pin {
	constructor(options) {
		...
		for (var key in this.defaultOptions) {
			...
			else this[key] = this.defaultOptions[key];
		}
